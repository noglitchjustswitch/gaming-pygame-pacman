...
pygame.display.set_caption("Pinky vs Pac-Man with Power Pellet")
...
# Load Blinky frames
blinky_frames = [
    load_scaled(f'assets/ghost_images/pinkyright{n}.png') for n in [1, 2]
]
blinky_blue_frames = [
...
while running:
    ...
    if blinky_state == "eyes" and keys[pygame.K_SPACE]:
        if blinky_home_rect.collidepoint(blinky_rect.center):
            blinky_state = "normal"
            print("Pinky manually respawned inside home base.")
    ...
    if not power_pellet_eaten and pacman_rect.colliderect(power_pellet_rect):
        ...
        print("Power pellet eaten! Pinky is vulnerable!")
